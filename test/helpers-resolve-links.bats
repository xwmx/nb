...
@test "'_resolve_links --browse' with .org includes pagination params in tag links." {
  ...
  printf "%s\\n" "${output}" | grep -q \
    "Selector link one: \[\[http://localhost:6789/home:2/1?--page=123&--per-page=2&--terminal\]\[\[\[home:2/1\]\]\]\] •"

  printf "%s\\n" "${output}" | grep -q \
    "Selector link two: \[\[http://localhost:6789/home:3?--page=123&--per-page=2&--terminal\]\[\[\[Root Title One\]\]\]\] •"

  printf "%s\\n" "${output}" | grep -q \
    "Tag one: \[\[http://localhost:6789/home:?--page=123&--per-page=2&--terminal&--query=%23tag_1\]\[#tag_1\]\] •"

  printf "%s\\n" "${output}" | grep -q \
    "Tag two: \[\[http://localhost:6789/home:?--page=123&--per-page=2&--terminal&--query=%23tag-2\]\[#tag-2\]\] •"
  ...
  printf "%s\\n" "${output}" | grep -q \
    "^\[\[http://localhost:6789/home:?--page=123&--per-page=2&--terminal&--query=%23tag3\]\[#tag3\]\]$"
}
