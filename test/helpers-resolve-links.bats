...
@test "'_resolve_links --browse' with .html includes pagination params in tag links." {
  ...
  printf "%s\\n" "${output}" | grep -q \
    "Tag one: <a href=\"http://localhost:6789/home:?--query=%23tag_1&--page=123&--per-page=2\">#tag_1</a> •"

  printf "%s\\n" "${output}" | grep -q \
    "Tag two: <a href=\"http://localhost:6789/home:?--query=%23tag-2&--page=123&--per-page=2\">#tag-2</a> •"

  printf "%s\\n" "${output}" | grep -q \
    "<p><a href=\"http://localhost:6789/home:?--query=%23tag3&--page=123&--per-page=2\">#tag3</a></p>"
}
...
@test "'_resolve_links --browse' with .org includes pagination params in tag links." {
  ...
  printf "%s\\n" "${output}" | grep -q \
    "Tag one: \[\[http://localhost:6789/home:?--query=%23tag_1&--page=123&--per-page=2\]\[#tag_1\]\] •"

  printf "%s\\n" "${output}" | grep -q \
    "Tag two: \[\[http://localhost:6789/home:?--query=%23tag-2&--page=123&--per-page=2\]\[#tag-2\]\] •"
  ...
  printf "%s\\n" "${output}" | grep -q \
    "^\[\[http://localhost:6789/home:?--query=%23tag3&--page=123&--per-page=2\]\[#tag3\]\]$"
}
